...
const CustomSlider: React.FC = () => {
  ...
  return (
    <div className="custom-slider" onMouseEnter={stopAutoPlay} onMouseLeave={startAutoPlay}>
      {/* Кнопки переключения */}
      <button className="custom-slider__button custom-slider__button--prev" onClick={handlePrev}>
        &#10094; {/* Левая стрелка */}
      </button>
      ...
      <button className="custom-slider__button custom-slider__button--next" onClick={handleNext}>
        &#10095; {/* Правая стрелка */}
      </button>
      ...
      <div className="custom-slider__indicators">
        {images.map((_, index) => (
          <span
            key={index}
            className={`custom-slider__indicator ${index === currentIndex ? 'custom-slider__indicator--active' : ''}`}
